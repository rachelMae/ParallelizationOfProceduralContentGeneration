...
public class PerlinNoiseGenerator {
    ...
    public float[][][] generatePerlinNoise3D(int width, int height, int depth, double scale) {
        float[][][] perlinArray = new float[width][height][depth];

        for (int x = 0; x < width; x++) {
            for (int y = 0; y < height; y++) {
                for (int z = 0; z < depth; z++) {
                    float value = (float) perlin.noise(x * scale, y * scale, z * scale);
