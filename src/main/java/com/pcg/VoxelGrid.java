...
public class VoxelGrid {
    ...
    public static void march_cube(int x, int y, int z, VoxelGrid voxel_grid, ArrayList<Vector3f> positions, int thread_id) {

        try {
            int[] triangulation = get_triangulation(x, y, z, voxel_grid);
            for (int i = 0; i < triangulation.length; i++) {
                // System.out.println("Thread " + thread_id + " triangulation[" + i + "]: " + triangulation[i]);
            ...
            System.out.println(Thread.currentThread().getId()  + " Exception is caught inside march_cube using coords x: " + x + ", y: " + y + ", z: " + z + " : " + e);
        }
    ...
    public static int[] get_triangulation(int x, int y, int z, VoxelGrid voxel_grid) {
        ...
        for (int i = 0; i < 8; i++) {
            // System.out.println(voxel_grid.read(x + TriTable.POINTS[i][0], y + TriTable.POINTS[i][1], z + TriTable.POINTS[i][2]) );
            if (voxel_grid.read(x + TriTable.POINTS[i][0], y + TriTable.POINTS[i][1], z + TriTable.POINTS[i][2]) < TriTable.SURFACE_LEVEL) {
                cube_index |= 1 << i;
                // System.out.println("Voxel grid value: " + voxel_grid.read(x + TriTable.POINTS[i][0], y + TriTable.POINTS[i][1], z + TriTable.POINTS[i][2]) + " is below than the surface level: " + TriTable.SURFACE_LEVEL);
            } else {
                // System.out.println("Voxel grid value: " + voxel_grid.read(x + TriTable.POINTS[i][0], y + TriTable.POINTS[i][1], z + TriTable.POINTS[i][2]) + " is greater than the surface level: " + TriTable.SURFACE_LEVEL);
            }
        }
